...
use amira_rust_asr_server::asr::simd::{
    batch_normalize_optimized, dot_product_optimized, softmax_optimized,
};
use amira_rust_asr_server::performance::{
    numa_allocate_vec, numa_aware::global_numa_allocator, AffinityManager, SpecializedExecutor,
    ThreadType,
};
...
async fn demo_cpu_affinity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Affinity supported: {}",
        affinity_manager.is_affinity_supported()
    );
...
async fn demo_lockfree_memory_pools() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Audio buffers hit rate: {:.1}%",
        final_stats.audio_buffers.hit_rate()
    );
    println!(
        "  Encoder inputs hit rate: {:.1}%",
        final_stats.encoder_inputs.hit_rate()
    );
...
async fn demo_specialized_thread_pools() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let executor =
        SpecializedExecutor::new().map_err(|e| format!("Failed to create executor: {}", e))?;
...
async fn demo_numa_awareness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..4 {
        let handle = tokio::spawn(async move {
            ...
            println!(
                "  Task {}: Allocated vectors of {} and {} elements",
                i,
                vec1.capacity(),
                vec2.capacity()
            );
