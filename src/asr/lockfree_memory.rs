...
use crate::constants::{audio, model, performance as config, triton};
...
impl<T> LockFreeObjectPool<T> {
    ...
    pub fn get(&self) -> LockFreePooledObject<'_, T> {
...
impl<'a, T> std::ops::Deref for LockFreePooledObject<'a, T> {
    ...
    fn deref(&self) -> &Self::Target {
        match self.obj.as_ref() {
            ...
            None => {
                error!(
                    "Attempted to dereference taken LockFreePooledObject - this indicates a bug"
                );
    ...
    pub fn new() -> Self {
        Self {
            audio_buffers: LockFreeObjectPool::new(
                || {
                    Vec::with_capacity(
                        audio::SAMPLE_RATE.value() as usize * config::AUDIO_BUFFER_SECONDS,
                    )
                },
...
pub fn get_lockfree_decoder_workspace(
) -> LockFreePooledObject<'static, crate::asr::zero_copy::DecoderWorkspace> {
