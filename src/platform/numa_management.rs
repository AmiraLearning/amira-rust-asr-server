...
use crate::performance::numa_aware::{NumaTopology, NumaNode};
...
impl NumaManager {
    ...
    fn initialize(&mut self) {
        ...
        if let Some(cloud_info) = &self.cloud_info {
            if self.should_disable_numa_for_cloud(cloud_info) {
                ...
                info!("NUMA disabled for cloud environment: {:?}", cloud_info.provider);
                return;
            }
        }
        ...
        match self.detect_numa_topology() {
            Ok(topology) => {
                if topology.numa_available && topology.nodes.len() > 1 {
                    ...
                    info!("NUMA enabled with {} nodes", self.topology.as_ref().unwrap().nodes.len());
                } else {
    ...
    fn should_disable_numa_for_cloud(&self, cloud_info: &CloudInstanceInfo) -> bool {
        match cloud_info.provider {
            ...
            CloudProvider::DigitalOcean | CloudProvider::Linode | CloudProvider::Vultr => {
                ...
                info!("Disabling NUMA for cloud provider: {:?}", cloud_info.provider);
                true
            }
            CloudProvider::Oracle => {
                // Oracle Cloud has bare metal instances that support NUMA
                if let Some(instance_type) = &cloud_info.instance_type {
                    let is_bare_metal = instance_type.contains("BM") || instance_type.contains("BareMetal");
                    if is_bare_metal {
                        info!("Enabling NUMA for Oracle bare metal instance: {}", instance_type);
                        false
                    } else {
    ...
    fn detect_numa_topology(&self) -> Result<NumaTopology> {
        ...
        let total_cores: usize = topology.cores_per_node.values()
            .map(|cores| cores.len())
            .sum();
        ...
        info!(
            ...
            topology.nodes.len(), total_cores
        );
    ...
    pub fn get_numa_config_recommendations(&self) -> NumaConfigRecommendations {
        ...
        if let Some(topology) = &self.topology {
            ...
            recommendations.reason = format!(
                ...
                topology.nodes.len(),
                topology.cores_per_node.values().map(|v| v.len()).sum::<usize>()
            );
        }
...
mod tests {
    ...
    fn test_numa_manager_creation() {
        ...
        assert!(matches!(
            manager.strategy(),
            NumaStrategy::Enabled | NumaStrategy::DisabledUnsupported | 
            NumaStrategy::DisabledCloud | NumaStrategy::Fallback
        ));
    }
