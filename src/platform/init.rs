...
use super::{
    ...
    cloud_detection::{
        apply_cloud_config, detect_cloud_environment, generate_cloud_config, CloudInstanceInfo,
    },
    detection::{detect_platform, IoBackendType, PlatformInfo, VirtualizationEnvironment},
    io_backend::{
        create_specific_io_backend, AsyncStream, CpuEfficiencyProfile, IoBackend, LatencyProfile,
        MemoryUsageProfile, PerformanceProfile, ThroughputProfile,
    },
...
fn apply_platform_optimizations(
    ...
    if config.force_io_backend.is_none() {
        let recommended_backend = match platform.virtualization {
            VirtualizationEnvironment::AWS | VirtualizationEnvironment::GCP => {
                if *capabilities
                    .feature_flags
                    .get("prefer_epoll_over_io_uring")
                    .unwrap_or(&false)
    ...
    if numa_recommendations.disable_numa_allocation {
        info!(
            "NUMA optimizations disabled: {}",
            numa_recommendations.reason
        );
    ...
    if affinity_recommendations.disable_affinity {
        info!(
            ...
            affinity_recommendations.reason
        );
        config.disable_cpu_affinity = true;
    } else {
        info!(
            "CPU affinity optimizations enabled: {}",
            affinity_recommendations.reason
        );
...
impl IoBackend for DummyBackend {
    async fn accept(&self) -> Result<(Box<dyn AsyncStream>, SocketAddr)> {
        ...
        Err(AppError::Network(
            "DummyBackend::accept should not be called".to_string(),
        ))
    }
...
fn validate_platform_config(
    ...
    if total_buffer_memory > 100 * 1024 * 1024 {
        // 100MB
        warn!(
...
fn log_effective_configuration(config: &Config, capabilities: &PlatformCapabilities) {
    ...
    info!(
        ...
        config.enable_platform_optimizations
    );
    ...
    info!(
        "Audio buffer capacity: {} KB",
        config.audio_buffer_capacity / 1024
    );
