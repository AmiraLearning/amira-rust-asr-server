...
fn detect_io_capabilities(platform: &PlatformInfo) -> IoCapabilities {
    ...
    match platform.os {
        OperatingSystem::Linux => {
            multiplexing.extend([IoMultiplexing::Select, IoMultiplexing::Poll, IoMultiplexing::Epoll]);
        ...
        OperatingSystem::MacOS | OperatingSystem::FreeBSD => {
            multiplexing.extend([IoMultiplexing::Select, IoMultiplexing::Poll, IoMultiplexing::Kqueue]);
        },
    ...
    IoCapabilities {
        io_uring_support,
        multiplexing,
        direct_io: matches!(platform.os, OperatingSystem::Linux | OperatingSystem::FreeBSD),
...
fn detect_simd_capabilities(platform: &PlatformInfo) -> Vec<SimdInstructionSet> {
    ...
    match platform.architecture {
        super::detection::Architecture::X86_64 => {
            ...
            {
                if std::is_x86_feature_detected!("sse") { simd.push(SimdInstructionSet::SSE); }
                if std::is_x86_feature_detected!("sse2") { simd.push(SimdInstructionSet::SSE2); }
                if std::is_x86_feature_detected!("sse3") { simd.push(SimdInstructionSet::SSE3); }
                if std::is_x86_feature_detected!("ssse3") { simd.push(SimdInstructionSet::SSSE3); }
                if std::is_x86_feature_detected!("sse4.1") { simd.push(SimdInstructionSet::SSE41); }
                if std::is_x86_feature_detected!("sse4.2") { simd.push(SimdInstructionSet::SSE42); }
                if std::is_x86_feature_detected!("avx") { simd.push(SimdInstructionSet::AVX); }
                if std::is_x86_feature_detected!("avx2") { simd.push(SimdInstructionSet::AVX2); }
                if std::is_x86_feature_detected!("avx512f") { simd.push(SimdInstructionSet::AVX512F); }
                if std::is_x86_feature_detected!("avx512bw") { simd.push(SimdInstructionSet::AVX512BW); }
                if std::is_x86_feature_detected!("avx512dq") { simd.push(SimdInstructionSet::AVX512DQ); }
            }
        },
...
fn detect_numa_capabilities(platform: &PlatformInfo) -> NumaCapabilities {
    ...
    let numa_likely_useful = matches!(platform.virtualization, VirtualizationEnvironment::BareMetal) &&
...
fn detect_memory_capabilities(platform: &PlatformInfo) -> MemoryCapabilities {
    ...
    MemoryCapabilities {
        large_pages,
        memory_locking: matches!(platform.os, OperatingSystem::Linux | OperatingSystem::FreeBSD),
...
fn detect_large_page_support(platform: &PlatformInfo) -> LargePageSupport {
    match platform.os {
        OperatingSystem::Linux => {
            ...
            let pages_2mb = std::path::Path::new("/sys/kernel/mm/hugepages/hugepages-2048kB").exists();
            let pages_1gb = std::path::Path::new("/sys/kernel/mm/hugepages/hugepages-1048576kB").exists();
            let transparent_huge_pages = std::path::Path::new("/sys/kernel/mm/transparent_hugepage").exists();
...
fn detect_network_capabilities(platform: &PlatformInfo) -> NetworkCapabilities {
    NetworkCapabilities {
        ...
        so_reuseport: matches!(platform.os, OperatingSystem::Linux | OperatingSystem::FreeBSD),
...
fn generate_feature_flags(
    ...
    // I/O feature flags
    flags.insert("io_uring".to_string(), !matches!(io.io_uring_support, IoUringSupport::None));
    flags.insert("io_uring_stable".to_string(), matches!(io.io_uring_support, IoUringSupport::Stable | IoUringSupport::Advanced | IoUringSupport::Full));
    flags.insert("epoll".to_string(), io.multiplexing.contains(&IoMultiplexing::Epoll));
    flags.insert("kqueue".to_string(), io.multiplexing.contains(&IoMultiplexing::Kqueue));
    
    // CPU feature flags
    flags.insert("simd_avx2".to_string(), cpu.simd.contains(&SimdInstructionSet::AVX2));
    flags.insert("simd_avx512".to_string(), cpu.simd.contains(&SimdInstructionSet::AVX512F));
    ...
    flags.insert("transparent_huge_pages".to_string(), memory.large_pages.transparent_huge_pages);
    ...
    flags.insert("cloud_environment".to_string(), !matches!(platform.virtualization, VirtualizationEnvironment::BareMetal));
    flags.insert("container".to_string(), matches!(platform.virtualization, VirtualizationEnvironment::Docker));
    
    // Optimization recommendation flags
    flags.insert("disable_numa_in_cloud".to_string(), 
        !matches!(platform.virtualization, VirtualizationEnvironment::BareMetal) && cpu.numa.available);
    flags.insert("prefer_epoll_over_io_uring".to_string(),
        matches!(platform.virtualization, VirtualizationEnvironment::AWS | VirtualizationEnvironment::GCP));
